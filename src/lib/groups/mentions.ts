...
const MENTION_RE =
  /(?:^|[\s([{])@(?:prava|aidhd|ai[\s_-]?dhd|ai|bot|concierge)\b/i;

const AGENT_HANDLES = ["prava", "aidhd", "ai", "bot", "concierge"];

/** True when the message tags @Prava (Meta-AI style) or is a slash command. */
export function mentionsAidhd(text: string): boolean {
  const t = text.trim();
  if (!t) return false;
  if (MENTION_RE.test(` ${t}`)) return true;
  if (/^(?:\/prava|\/aidhd|hey prava|hey aidhd|oi aidhd)\b/i.test(t)) return true;
  return false;
}
...
export function stripMentions(text: string): string {
  return text
    .replace(/@(?:prava|aidhd|ai[\s_-]?dhd|ai|bot|concierge)\b/gi, "")
    .replace(/^\s*\/(?:prava|aidhd)\b/i, "")
    .replace(/\s+/g, " ")
    .trim();
}
