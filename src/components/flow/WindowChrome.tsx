...
export function WindowChrome({ path, children }: { path: string; children: ReactNode }) {
  ...
  return (
    <div className="overflow-hidden rounded-2xl border border-line bg-surface shadow-card shadow-card">
      <div className="flex items-center gap-3 border-b border-line bg-canvas/80 px-3 py-2.5 sm:px-4 sm:py-3">
        <div className="flex shrink-0 gap-1.5 sm:gap-2">
          <span className="h-2 w-2 rounded-full bg-ink sm:h-2.5 sm:w-2.5" />
          <span className="h-2 w-2 rounded-full bg-ink sm:h-2.5 sm:w-2.5" />
