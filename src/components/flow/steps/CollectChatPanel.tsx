    ...
    <div className="flex h-[26rem] flex-col">
      ...
      <div className="mt-3 flex items-center gap-2">
        <input
          ...
          className="focus-ring flex-1 rounded-full border border-line bg-surface px-4 py-2.5 text-sm text-ink transition focus:border-coral-dark disabled:opacity-60"
        />
        <button
          type="button"
          onClick={submit}
          disabled={!draft.trim() || invitee.agentTyping}
          className="shrink-0 rounded-full bg-coral px-5 py-2.5 text-sm font-bold text-ink transition disabled:cursor-not-allowed disabled:opacity-40"
        >
          Send
        </button>
      </div>
